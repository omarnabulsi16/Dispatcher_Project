...
/**
 * @description Inline editor for HTML 

tables compatible with Bootstrap
 * @version 1.2.3
            ...
            columns: {
                identifier: function() {
                    ...
                    if (settings.hideIdentifier) {
                        $table.find('th:nth-child(' + parseInt

(settings.columns.identifier[0]) + 1 + '), tbody td:nth-child(' + parseInt(settings.columns.identifier[0]) + 1 + ')').hide();
                    }

                    var $td = $table.find('tbody td:nth-child(' + (parseInt

(settings.columns.identifier[0]) + 1) + ')');

                    $td.each(function() {
                        // Create hidden input with row identifier.
                        var span = '<span class="tabledit-span tabledit-

identifier">' + $(this).text() + '</span>';
                  ...
                  // Add elements to table cell.
                        ...
                        $(this).parent('tr').attr(settings.rowIdentifier, 
...
$(this).text());
                ...
                editable: function() {
                    for (var i = 0; i < settings.columns.editable.length; i++) {
                        var $td = $table.find('tbody td:nth-

child(' + (parseInt(settings.columns.editable[i][0]) + 1) + ')');
       ...
       // Add pointer as cursor.
                      ...
                      var span = '<span class="tabledit-span">' + text + '</span>';
                            ...
                            if (typeof settings.columns.editable[i]
...
input += '<option value="' + index + '">' + value + '</option>';
 ...
 input += '</select>';
                                ...
                                var input = '<input class="tabledit-input ' + settings.inputClass + '" type="text" 

name="' + settings.columns.editable[i][1] + '" value="' + $(this).text() + '" style="display: none; width:20px;" disabled>';
                            }
...
(settings.editButton || settings.deleteButton) {
                        var editButton = '';
                        var deleteButton = '';
                        var saveButton = '';
                        var restoreButton = '';
...
('tr:first').append('<th class="tabledit-toolbar-column"></th>');
                        }
...
type="button" class="tabledit-edit-button ' + settings.buttons.edit.class + '" style="float: none;">' + settings.buttons.edit.html + '</button>';
                        }
...
settings.buttons.save.class + '" style="display: none; float: none;">' + settings.buttons.save.html + '</button>';
                        }
...
&& settings.restoreButton) {
                            restoreButton = '<button type="button" class="tabledit-restore-button ' + settings.buttons.restore.class + '" style="display: none; float: none;">' + settings.buttons.restore.html + 
...
settings.groupClass + '" style="float: none;">' + editButton + deleteButton + '</div>\n\
                                           ' + saveButton + '\n\
                                           ' + confirmButton + '\n\
                        ...
                        $table.find('tr:gt(0)').append('<td style="white-space: 

nowrap; width: 1%;">' + toolbar + '</td>');
                    }
                }
            }
        };
...
*/
        var Mode = {
            view: function(td) {
                ...
                var $tr = $(td).parent('tr');
                // Disable identifier.
                $(td).parent('tr').find('.tabledit-input.tabledit-
...
class and remove "view" class in td element.
...
$tr.find('button.tabledit-edit-button').addClass('active');
        ...
        /**
         * Available actions for edit function, with table 

td element as parameter or set of td elements.
         *
         * @type object
         */
        var Edit = {
...
('.tabledit-span').text($input.find('option:selected').text());
                    } else {
        ...
        /**
         * Available actions for delete function, with 
...
(settings.buttons.delete.action);
  ...
  return;
                ...
                // Hide table row.
                $(td).parent('tr').addClass

(settings.mutedClass).find('.tabledit-toolbar button:not(.tabledit-restore-button)').attr('disabled', true);
                // Show restore button.
...
$(td).find('.tabledit-confirm-button').show();
            ...
            restore: function(td) {
                // Enable identifier hidden input.
                $(td).parent('tr').find('input.tabledit-identifier').attr('disabled', 

false);
                // Send AJAX request to server.
                var ajaxResult = ajax(settings.buttons.restore.action);
                // Disable identifier hidden input.
                $(td).parents('tr').find('input.tabledit-
...
identifier').attr('disabled', true);
                ...
                $(td).parent('tr').removeClass('tabledit-
...
('tr.' + settings.warningClass).removeClass(settings.warningClass);
                    }, 1400);
                }
...
('.tabledit-toolbar button').attr('disabled', false);
...
var $button = $(this);
       ...
       Edit.reset($table.find('td.tabledit-edit-mode'));
                    ...
                    if (!activated) {
                        ...
                        $($button.parents('tr').find

('td.tabledit-view-mode').get().reverse()).each(function() {
                            Mode.edit(this);
                        });
                    }
...
/**
     ...
     event.preventDefault();
             ...
             * @param {object} event
             */
            $table.on(settings.eventType, 'tr:not(.tabledit-deleted-row) 

td.tabledit-view-mode', function(event) {
                if (event.handled !== true) {
                    event.preventDefault();

                    // Reset all td's in edit mode.
                    Edit.reset($table.find('td.tabledit-
...
edit-mode'));
            ...
            /**
             * Change event when input is a select 
...
$(document).on('click', function(event) {
                ...
                if (!$editMode.is(event.target) && $editMode.has
...
$table.find('.tabledit-confirm-button');
